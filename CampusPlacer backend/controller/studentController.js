...
const Register = async (req, res) => {
    try {
        ...
        const { s_name, s_phone, s_address, s_email, s_password, register_no, branch_id, role_id } = req.body
        ...
        if (check.length > 0) {
            ...
            //console.log(secpass)
            const student = await studentSchema({ s_name, s_phone, s_address, s_email, register_no, branch_id, role_id, s_password: secpass, s_photo: photo, s_resume: resume })
            await student.save()
...
const Get = async (req, res) => {
    try {
        ...
        const student = await studentSchema.find().populate(["role_id", "branch_id"])
...
const GetById = async (req, res) => {
    try {
        const id = req.params.id
        const student = await studentSchema.findById(id).populate(["role_id", "branch_id"])
        res.json({ success: true, student })
...
const Update = async (req, res) => {
    try {
        ...
        if (!check) {
            ...
            const { s_name, s_phone, s_address, s_email, register_no, branch_id } = req.body
            const newData = {}
