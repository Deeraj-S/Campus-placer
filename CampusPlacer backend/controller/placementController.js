...
const placementRegister = async (req, res) => {
    try {
        ...
        const { p_name, p_phone, p_email, role_id, p_password, p_photo } = req.body
        ...
        if (check.length > 0) {
            ...
            const placement_officer = await placementSchema({ p_name, p_phone, p_email, role_id, p_password: secpass, p_photo: req.file.filename })
...
const Get = async (req, res) => {
    try {
        const placement_officer = await placementSchema.find().populate(["role_id"])
...
const GetById = async (req, res) => {
    try {
        ...
        const placement_officer = await placementSchema.findById(id).populate(["role_id"])
        res.json({ success: true, placement_officer })
